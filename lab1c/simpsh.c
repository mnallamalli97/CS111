...
struct timeval usr_s;
struct timeval usr_f;
struct timeval ker_s;
struct timeval ker_f;

// for the shell's wait/for the maximum return code
int shell_wstatus;
...
enum command_output {
  _not_output = 0, _output_done
};
typedef enum command_output command_output;
...
/* update current execution times
 * */
void profile_function(const char *command_display)
{
    /* start the kenerl and user time, if cannot getrusage, repport error */
    int rusage_val = 0;

    /* test the global flag for profiling
     * */
    if(profile_f != 1)
    {
	    return;
    ...
    rusage_val = getrusage(RUSAGE_SELF, &usage);

    if (rusage_val == -1)
    {
    	fprintf(stderr, "Cannot run the usage %s\n", strerror(errno));
    }

    usr_f = usage.ru_utime;
    ker_f = usage.ru_stime;

    userTime = ((double)usr_f.tv_sec + (double)usr_f.tv_usec/1000000)
	    - ((double)usr_s.tv_sec + (double)usr_s.tv_usec/1000000);
    kernalTime = ((double)ker_f.tv_sec + (double)ker_f.tv_usec/1000000) 
	    - ((double)ker_s.tv_sec + (double)ker_s.tv_usec/1000000);

    fprintf(stdout, " %s - User: %f, Kernel: %f \n", command_display, userTime,
	kernalTime);
}

void profile_reset_start_times()
{
    /* update the global start time usage information
     * */
    int rusage_val = 0;
    ...
    usr_s = usage.ru_utime;
    ker_s = usage.ru_stime;
}
...
int main(int argc, char **argv)
{
    ...
    /* initialize the shell's return code
     * */
    shell_wstatus = 0;
 ...
 * */
	case 'r':
	    ...
	    profile_function("profile: --rdonly");
 ...
 * */
	case 'w':
	    ...
	    profile_function("profile: --wronly");
...
// discovered running case 20 with the excl_f set
//	    newfile = open(argv[optind - 1],
 ...
 * */
	case 'a':
	    ...
	    profile_function("profile: --append");
 ...
 * */
	case 'b':
	    ...
	    profile_function("profile: --cloexec");
 ...
 * */
	case 'd':
	    ...
	    profile_function("profile: --creat");
 ...
 * */
	case 'e':
	    ...
	    profile_function("profile: --directory");
 ...
 * */
	case 'f':
	    ...
	    profile_function("profile: --dsync");
 ...
 * */
	case 'g':
	    ...
	    profile_function("profile: --execl");
 ...
 * */
	case 'h':
	    ...
	    profile_function("profile: --nofollow");
 ...
 * */
	case 'i':
	    ...
	    profile_function("profile: --nonblock");
 ...
 * */
	case 'j':
	    ...
	    profile_function("profile: --rsync");
 ...
 * */
	case 'l':
	    ...
	    profile_function("profile: --trunc");
 ...
 * */
	case 'c':		//command
	    ...
	    profile_function("profile: --command");
 ...
 * */
	case 'v':
	    ...
	    profile_function("profile: --verbose");
 ...
 * */
	case 'm':
	    ...
	    profile_function("profile: --abort");
 ...
 * */
	case 'o':
	    ...
	    profile_function("profile: --pause");
 ...
 * */
	case 'p':
	    ...
	    profile_function("profile: --catch");
 ...
 * */
	case 'q':
	    ...
	    profile_function("profile: --close");
 ...
 * */
	case 'u':
	    ...
	    profile_function("profile: --ignore");
 ...
 * */
	case 'z':
	    ...
	    // now that the flag is processed collect timing information
	    profile_function("profile: --default");
	...
	if (wait_f == 1)
		...
		if(0 < cmd_ind)
		{
			...
			while(i < cmd_ind)
			{
				...
				if( ! WIFCONTINUED(command_list[i].wstatus) 
				&& ! WIFSTOPPED(command_list[i].wstatus))
				{
					if( command_list[i].report_f != _output_done) 
					{
				    		if (WIFSIGNALED(command_list[i].wstatus))
						{
							fprintf(stdout, "1126 signal %d %s ",
								WTERMSIG(command_list[i].wstatus),
								command_list[i].name);

							/* record the larger of the signal or the
							 * current shell_wstatus
							 * */
							if(shell_wstatus < WTERMSIG(command_list[i].wstatus))
							{
								shell_wstatus = WTERMSIG(command_list[i].wstatus);
							}
						}
						else if(WIFEXITED(command_list[i].wstatus))
						{
							fprintf(stdout, "1140 exit %d %s ",
								WEXITSTATUS(command_list[i].wstatus),
								command_list[i].name);

							/* record the larger of the exit status or the
							 * current shell_wstatus
							 * */
							if(shell_wstatus < WEXITSTATUS(command_list[i].wstatus))
							{
								shell_wstatus = WEXITSTATUS(command_list[i].wstatus);
							}
						}
					 ...
					 * */
					command_list[i].wait_f = _done_waiting;

					/* the shell has reported on this process
					 * */
					command_list[i].report_f = _output_done;
		 ...
		 * */
		profile_function("profile and wait in child processing loop");
		wait_f = 0;
	}
...
/* ****************************************************************************
 * check on the child processes if any to get the proper return code
 *
 * initialized at the top of the option processing loop
    shell_wstatus = 0;
 * */
 ...
 * */
	if(wait_f == 1 && command_list[i].wait_f != _done_waiting)
	{
		...
		profile_function("profile and wait" );
	...
	//profile_function("profile and not wait" );
    }
