...
void analyze_server(char* buff){
	int ans = SSL_read(ssl, buff, 256);
	char* temp = buff;
	if (ans != 0)
	{
		buff[ans] = 0;
...
int main(int argc, char** argv){
	...
	if (connect(sock, (struct socket_address*) &server_addy, sizeof(server_addy)) < 0)
	{
	...
	if (ssl == NULL)
	{
		fprintf(stderr, "can't find ssl\n" );
		exit(2);
	}



	//after connecting the socket, can connect SSL
	int set_fd = SSL_set_fd(ssl, sock);
	if (set_fd != 1)
	{
		fprintf(stderr, "can't set the file direcrtory\n" );
		exit(2);
	}

	if (SSL_connect(ssl) != 1)
	{
		fprintf(stderr, "can't start TLS\n" );
		exit(1);
	}
	...
	char print[50];
	sprintf(print, "ID=%s\n", id);
	SSL_write(ssl, print, strlen(print));
	if (log_v_flag == 1)
	{
		fputs(print, log_v);
	}
	...
	for(;;){
		...
		if (start.tv_sec >= next_time && out_time_temp == 1)
		{
			...
			fputs(printt, stdout);
			SSL_write(ssl, printt, strlen(printt)); 
		...
		if (ret)
		{
			SSL_read(ssl, input_command, 128);
